                    ...
                    if hour >= shopConfig.shopClose or hour < shopConfig.shopOpen then
                        ...
                        if (distanceShop <= shopConfig.distanceShop) and not IsPedInAnyBoat(player) then
                            ...
                            if Citizen.InvokeNative(0xC92AC953F0A982AE, CloseShops) then -- UiPromptHasStandardModeCompleted
                                ...
                                Wait(100)
                                TriggerEvent('rNotify:NotifyLeft', "BOATS", "SORRY WE ARE CLOSED", "generic_textures", "tick", 4000)
                            end
                            ...
                            if Citizen.InvokeNative(0xC92AC953F0A982AE, OpenReturn) then -- UiPromptHasStandardModeCompleted
                                ...
                                if currentLocation == boatHome then
                                    ...
                                    if TransferAllow then
                                        ...
                                        ReturnBoat(shopId)
                                        TriggerEvent('rNotify:NotifyLeft', "BOATS", "ARE AVAILABLE", "generic_textures", "tick", 4000)
                                    else
                                        ReturnBoat(shopId)
                                        TriggerEvent('rNotify:NotifyLeft', "BOATS", "WERE RETURNED", "generic_textures", "tick", 4000)
                                    end
                                end
                            end
                        end
        ...
        if data.current.value == "anchor" then
            if IsPedInAnyBoat(player) then
                ...
                if not isAnchored then
                    ...
                    isAnchored = true
                    TriggerEvent('rNotify:NotifyLeft', "ANCHOR", "DEPLOYED DOWN", "generic_textures", "tick", 4000)
                else
                    SetBoatAnchor(playerBoat, false)
                    isAnchored = false
                    TriggerEvent('rNotify:NotifyLeft', "ANCHOR", "BROUGHT UP", "generic_textures", "tick", 4000)
                end
            end
            menu.close()
            InMenu = false
...
function SpawnBoat()
    ...
    IsBoating = true
    TriggerEvent('rNotify:NotifyLeft', "PLEASE USE", "'O' to open Boat Menu", "generic_textures", "tick", 6000)
end
